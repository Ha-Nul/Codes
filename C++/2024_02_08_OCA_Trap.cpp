...
{   
       ...
       if (n == 1)
    {
        ...
        for (int i = 0 ; i < n ; i++)
        {
            SUM += 0.5 * (Self_E[n-(i)] * Prop_ite[i] + Self_E[n-(i+1)] * Prop_ite[i+1]);
...
vector<MatrixXd> Testing::Propagator(const vector<MatrixXd> &Self_E, const MatrixXd &loc, const double &gvalue)
{
    ...
    MatrixXd Former = MatrixXd::Zero(3,3);
    MatrixXd Latter = MatrixXd::Zero(3,3);
...
int main()         
{
    ...
    */
    ...
    for (int k=0; k<g_array.size(); k++)
    {
        ...
        string name = "OCA_grid100_beta_1_g_";
