...
vector<double> Testing::green(double tau)
{
    ...
    for (int i = 0; i < k; i++)
    {
        bose_dist[i]=one_vec[i]/(exp(beta * k_grid[i])-1);
    }
...
vector<double> Testing::Interact(vector<double> coupling, vector<double> tau)
{
    ...
    for (int i = 0; i < k; i++){
        ...
        blank_matrix = MatrixXd::Zero(k,k_grid.size());
    }
...
MatrixXd Testing::round_propagater_ite(const MatrixXd &loc, const vector<MatrixXd> &sigma, const vector<MatrixXd> &ite, int n)
{
    ...
    for (int i = 0 ; i <= n ; i++)
    { 
        sigsum += (sigma[n-i] * ite[i] + sigma[n] * ite[i+1]);
        if (i+1 == n)
        {
            break;
        }
...
vector<MatrixXd> Testing::Propagator(const vector<MatrixXd> &array, const MatrixXd &loc)
{
    ...
    */
    proparray[0] = Iden;
    round_prop_bucket = MatrixXd::Zero(3,3);
    MatrixXd round_prop_xx = MatrixXd::Zero(3,3);
    MatrixXd bucket = MatrixXd::Zero(3,3);
    ...
    { 
        if (i>1)
        {
            round_prop_bucket = round_propagater_ite(loc,Sig,proparray,i-2);
            round_prop_xx = round_propagater_ite(loc,Sig,proparray,i-1);

            bucket = 0.5*(round_prop_bucket + round_prop_xx);
    
            proparray[i] = proparray[i-1] + (tau_grid[1]-tau_grid[0]) * 0.5 * (round_prop_bucket + round_prop_xx);
        }
            proparray[i] = proparray[i-1] + (tau_grid[1]-tau_grid[0]) * (round_propagater_ite(loc,Sig,proparray,i-1));
...
int main()
{
    ...
    /*
    vector<double> a = test.Interact_V(test.coupling(velocity,g,cutoff),test.grid,omega);
