...
void MD_OC::readVfunc(double gamma, double alpha)
{
    ...
    filename += ".dat";
    */
    ifstream readFile("./data/INT_Arr_g0.38_a0.55.dat");
...
MatrixXcd MD_OC::Make_N_Matrix(int lineNumber, double ng) {
    ...
    */
    ...
    ifstream Refile("./data/M_c_gam_0to0.4_0.5_Re.txt");
    ifstream Imfile("./data/M_c_gam_0to0.4_0.5_Im.txt");
...
MatrixXcd MD_OC::Make_Loc_Matrix(int lineNumber, double ng){
    ...
    */
    ...
    ifstream file("./data/M_c_gam_0to0.4_0.5_Im.txt");
    string line;
    ...
    for (int i = 0; i<al; i++){
        for(int j = 0; j<al; j++){
            ...
            if (outputFile.is_open()) { // 파일 열기 확인
                ...
                outputFile << MD.tau_grid[j] << "\t" << a[j].imag() << endl; // a[j].real()로 실수부 추출
            }
            outputFile.close();
            } else {
