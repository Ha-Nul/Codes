...
def Local_For_INT_odd_Eigenvec(gamma,n,m):
    ...
    for i in range(n):
        if i==0:
            INT_ARR_O.append(0) # to set same basis with even case (index [0] corresponds with sin(0\phi))
        else:
...
def INT_even_Eigenvec(gamma, n, m):
    ...
    for i in range(n):
        INT_ARR_E.append(1j * i * LOC_VEC_E[i]) # Once differentiate, minus sign appears and then vanishes because of the factor (-i)
...
def Hamiltonian_Matrix(gamma: float,n: int, g: float,omega: float):
    ...
    # Eigenvectors for construct the N matrix
