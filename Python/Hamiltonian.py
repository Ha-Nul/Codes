...
def Hamiltonian_Matrix(r: float,z: int,g: float,omega: float):
    ...
    #Matrix Elements '''Eigenval: 2*2기준에서 계산됨'''
    eig_even_gs = Standard_even_Eigenval(r,3,0)
    eig_even_2n = Standard_even_Eigenval(r,3,1)
    eig_odd = Standard_odd_Eigenval(r,3,0)

    Diag_gs_to_1s = np.dot(Standard_odd_Eigenvec(r,3,0),Standard_even_Eigenvec(r,3,0))
    Diag_1s_to_2n = -np.dot(Standard_odd_Eigenvec(r,3,0),Standard_even_Eigenvec(r,3,1))
...
def Chi_sp(beta : float, r: float, omega: float, g: float, z: int , tau: float):
    ...
    for i in range(3*z):
        for j in range(3*z):
            ...
            Expec_NM = Elements(Lie_tensorproduct(1,z),Hamiltonian_Matrix_Eigenvec(r,3*z,g,omega,i),Hamiltonian_Matrix_Eigenvec(r,3*z,g,omega,j))
            #Expec_NM = Hamiltonian_Matrix_Eigenvec(r,3*z,g,omega,j).T @ Lie_group(1) @ Hamiltonian_Matrix_Eigenvec(r,3*z,g,omega,i)
